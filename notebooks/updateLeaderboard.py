...
'''
  ...
  for f in range(evalResults['MAUC'].shape[0]):
    if not np.isnan(evalResults['MAUC'].iloc[f]):
      text += '\n   <tr class="d%d">' % (f % 2)
      teamName = forecastFiles[f].split('.')[0][len('TADPOLE_Submission_Pycon_'):]
...
def downloadLeaderboardSubmissions():
  ...
  fileListLdb = [x for x in fileListAll.keys() if x.startswith('TADPOLE_Submission_Pycon')]
  fileListLdb.sort()
