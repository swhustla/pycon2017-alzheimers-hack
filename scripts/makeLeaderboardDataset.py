...
tadpoleDf.insert(loc=6, column='LB1', value=np.nan)
tadpoleDf.insert(loc=7, column='LB2', value=np.nan)
tadpoleDf['LB1'] = LB1
tadpoleDf['LB2'] = LB2

# filter out entries not in LB1 or LB2, in order to ensure participants don't use these by mistake
tadpoleDf = tadpoleDf[np.logical_or(tadpoleDf['LB1'] == 1, tadpoleDf['LB2'] == 1)]
tadpoleDf.reset_index(drop=True, inplace=True)

tadpoleDf.to_csv('TADPOLE_LB1_LB2.csv',index=False)
print('TADPOLE_LB1_LB2.csv created in local folder')

print('columns', list(tadpoleDf.columns.values)[:10])
# print(adsad)
